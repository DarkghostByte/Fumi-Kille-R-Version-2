  ...
  <div>
    <div class="container mx-auto px-4">
      ...
      <div class="flex justify-between items-center mb-4">
        <h1 class="text-2xl font-semibold">Creditos</h1>
        <div class="py-3">
          <router-link to="/admin/home" class="el-button el-button--danger">
      ...
      <div class="flex" style="justify-content: center;">
        ...
        <el-table-column label="">
          <template #default="scope">
            <router-link :to="'/admin/works/edit-workCreditComplete/'+scope.row.id">
              <el-button style="color:black"
                size="small"
                type="warning"
                @click="handleEdit()"
  ...
  methods: {
    refresh() {
      axios.get('completarOrden').then(res => {
        this.tableData = res.data.data.filter(row => row.requiere3 === 'Credito');
    ...
    filterDataMonto() {
      this.filteredData = this.tableData.filter((completarOrden) => {
        return completarOrden.pago.toString().includes(this.searchQueryMonto.toLowerCase());
      });
    },
